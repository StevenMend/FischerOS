// src/auth/forms/StaffAuthForm.tsx - REDISEÑADO CON PALETA RESORT
import React, { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { Users, Building2, ArrowLeft, AlertCircle, Clock, Zap, Eye, EyeOff } from 'lucide-react';
...
export default function StaffAuthForm() {
  ...
  const validateForm = () => {
    ...
    if (!formData.staffId.trim()) {
      errors.push('ID de staff es requerido');
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await loginAsStaff(formData.staffId, formData.department, formData.password, formData.property);
    } catch (err) {
