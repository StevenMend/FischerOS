// src/features/restaurants/staff/hooks/useRestaurantStaff.ts
...
export const useRestaurantStaff = (
  ...
  const seatReservation = async (reservationId: string, tableNumber?: string) => {
    await updateMutation.mutateAsync({
      reservationId,
      updates: { status: 'seated', table_number: tableNumber },
    });
  };
