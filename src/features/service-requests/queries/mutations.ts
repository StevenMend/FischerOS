...
export const useTakeRequestMutation = (departmentId: string, staffId: string) => {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      // Notify the guest their request is being handled
      if (data.guest_id) {
        notify({
          userId: data.guest_id,
          type: 'info',
          title: 'Request In Progress',
          body: `A team member has been assigned to your request "${data.title}".`,
...
export const useUpdateStatusMutation = (departmentId: string, guestId?: string) => {
  ...
  return useMutation({
    ...
    onSuccess: (data, { status }) => {
      ...
      if (status === 'completed' && data.guest_id) {
        queryClient.invalidateQueries({ queryKey: serviceRequestsKeys.byGuest(data.guest_id) });

        // Notify the guest their request was completed
        notify({
          userId: data.guest_id,
          type: 'success',
          title: 'Request Completed',
          body: `Your request "${data.title}" has been completed.`,
          category: 'service_request',
          relatedId: data.id,
          relatedType: 'service_request',
        });
